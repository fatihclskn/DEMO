...
namespace Business.Concrete
{
    public class ProductManager : IProductService
    {
        ...
        {
            IResult result = BusinessRules.Run(CheckCategoryCount(product.CategoryId), CheckProductNameExists(product.ProductName), ChekIfCategoryLimitExceded());
            if (result != null)
            {
                return result;
            }
        ...
        private IResult CheckCategoryCount(int categoryId)
        {
            ...
            if (result >= 10)
            {
                return new ErrorResult(Messages.CategoryLengthMuch);
            }
